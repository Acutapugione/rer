...
def create_lot():
    ...
    if request.method == "POST":
        ...
        for lot_image in lot_images:
            if (
                lot_image
                and allowed_file(lot_image.filename)
                and is_image_file(lot_image.stream)
            ):
                ...
                return render_template(
                    "create_lot.html", error="Потрібно завантажити зображення як файл"
                )
            ...
            for lot_image in lot_images:
                if (
                    ...
                    return render_template(
                        "edit_lot.html",
                        lot=lot,
                        error="Потрібно завантажити зображення як файл",
                    )
    ...
    if request.method == "GET":
        ...
        for lot_image in lot_images:
            if (
                ...
                return (
                    jsonify({"error": "Потрібно завантажити зображення як файл"}),
                    400,
                )
...
def place_bid():
    if "username" not in session:
        return jsonify({"error": "Необхідно увійти в систему"}), 403

    data = request.get_json()
    lot_id = data.get("lot_id")
    bid = data.get("bid")

    lot = find_lot_by_id(lot_id)
    if not lot:
        return jsonify({"error": "Лот не знайдено"}), 404

    if bid <= lot["current_price"]:
        return jsonify({"error": "Ставка повинна бути більше поточної ціни"}), 400

    lot["current_price"] = bid
    save_lots()
    data = {
        "success": True,
